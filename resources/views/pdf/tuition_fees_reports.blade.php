    ...
    <body>
        ...
        <h5 class="text-uppercase " style="text-align: center;margin:auto">
         ...
         <h5 class="text-uppercase">
            {{ trans('main.date')}} <span style="text-size:12px !important;margin:0 3px;">{{date('d-m-Y')}}</span>
            <br>
            @if(isset($academic_year_id)){{ trans_choice('main.academic_year',1)}} <span style="text-size:12px !important;margin:0 3px;">{{\App\Models\AcademicYear::find($academic_year_id)->name}}</span>@endif
            <br>
        ...
        <table class="w-ful" style="width: 100%" id="payment_list">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 border-b">
                ...
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 border">
                        {{trans('main.total_fees')}}
                    </th>
                    <th scope="col" class="px-6 py-3 border">
                        {{trans('main.totalPayment')}}
                    </th>
                    <th scope="col" class="px-6 py-3 border">
                        {{trans('main.current_balance')}}
                    </th>
                    <th scope="col" class="px-6 py-3 border">
                        {{trans('main.need_to_pay_balance')}}
                    </th>
                </tr>
            </thead>
            <tbody>
                ...
                @forelse ($students as $student)
                    <tr class="">
                        ...
                        <td class="px-6 py-4  border">
                            {{number_format(calculateAllFees($student)['totals']['grand_total'],2,',',',')." ".trans('main.'.env('DEFAULT_CURRENCY'))}}
                        </td>
                        <td class="px-6 py-4  border">
                            {{number_format($student->payments()    ,2,',',',')." ".trans('main.'.env('DEFAULT_CURRENCY'))}}
                        </td>
                        <td class="px-6 py-4  border">
                            {{number_format($student->opening_balance + calculateAllFees($student)['totals']['grand_total'] -$student->payments()     ,2,',',',') ." ".trans('main.'.env('DEFAULT_CURRENCY'))}}
                        </td>
                        <td class="px-6 py-4  border">
                            {{number_format($student->opening_balance + calculateAllFees($student)['totals']['total_fees_to_pay'] -$student->payments()     ,2,',',',') ." ".trans('main.'.env('DEFAULT_CURRENCY'))}}
                        </td>
                    </tr> 
